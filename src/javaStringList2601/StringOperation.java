...
//        result.forEach((i, j) -> System.out.println(i + " : " + j));
        ...
        Map<Character, Long> map2 = s2.toLowerCase().chars().mapToObj(c -> (char) c).collect(Collectors.groupingBy(i -> i, Collectors.counting()));
        map2.entrySet().stream().sorted(Map.Entry.<Character, Long>comparingByValue().reversed()).forEach(System.out::println);

        //first non-repeating char in string
       Map<Character, Long> map3 = s2.toLowerCase().chars().mapToObj(c->(char)c).collect(Collectors.groupingBy(i->i, Collectors.counting()));
        Character nonRepeatingKey = map3.entrySet().stream().filter(i -> i.getValue() == 1).findFirst().get().getKey();
        System.out.println("First non-repeating char in string:"+nonRepeatingKey);

        //first repeating char in string
        Character repeatingKey = map3.entrySet().stream().filter(i->i.getValue()>1).findFirst().get().getKey();
        System.out.println("First non-repeating char in string:"+repeatingKey);

        //reverse of string
        String s="hello world";
        String reduce = s.chars().mapToObj(c -> String.valueOf((char) c)).reduce("", (a, c) -> c + a);
